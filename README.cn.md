...
5. 默认情况下，每次 dispatch 完成后 rollover 群 session（stateless 模式），让每轮只有精炼的 GroupSense 上下文——不堆积 LLM 历史，不浪费 token。

当 `plugins.slots.contextEngine` 设为 `"groupsense"` 时，插件会跳过这个 post-dispatch rollover，改为由 context engine 向模型隐藏旧 transcript，同时保留真实物理 session，方便在 Control UI 里继续查看当前会话。

如果要直接核对真实送进 LLM 的 payload，可以临时打开：
...
打开后，插件会把 Feishu 群聊 turn 的真实 `llm_input` hook payload 写到 `~/.openclaw/shared-knowledge/feishu-groupsense-context-engine/llm-input-traces`，里面会包含 `systemPrompt`、`prompt` 和 `historyMessages`。

当 trace 落盘关闭时，gateway 仍会输出轻量级 `feishu[groupsense]` 日志，记录 checkpoint capture / checkpoint hit / delegate fallback / `llm_input` prompt marker，方便平时排查而不用把完整 payload 写盘。
