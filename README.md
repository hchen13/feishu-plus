...
| `sheets:spreadsheet:readonly` | `feishu_sheet` | Read spreadsheet metadata and cell ranges |
...
| `sheets:spreadsheet` | `feishu_sheet` | Write range values and append rows |
...
> **Important:** The bot can only read/write spreadsheets that have been **shared with it**.

To let the bot use `feishu_sheet`:
1. Open the spreadsheet you want the bot to access
2. Click **Share** button → search for your bot name
3. Grant appropriate permission (view/edit)

`feishu_sheet` supports token extraction from:
- `/sheets/XXX` (new URL)
- `/sheet/XXX` (legacy URL)
- `/wiki/XXX` (wiki sheet node, auto-resolves to spreadsheet token)
...
- **Sheet tools**: Read/write spreadsheet ranges and append rows without browser automation, supports `/sheets/`, `/sheet/`, and `/wiki/` links
...
| `sheets:spreadsheet:readonly` | `feishu_sheet` | 读取电子表格元信息和单元格范围 |
...
| `sheets:spreadsheet` | `feishu_sheet` | 写入单元格范围与追加行 |
...
> **重要：** 机器人只能读取/写入**已分享给它**的电子表格。

要让机器人使用 `feishu_sheet`：
1. 打开需要机器人访问的电子表格
2. 点击 **分享** 按钮 → 搜索机器人名称
3. 授予相应权限（查看/编辑）

`feishu_sheet` 支持从以下链接自动提取 token：
- `/sheets/XXX`（新链接）
- `/sheet/XXX`（旧链接）
- `/wiki/XXX`（知识库中的电子表格节点，自动解析为 spreadsheet token）
...
- **电子表格工具**：无需浏览器即可读取/写入单元格范围并追加行，支持 `/sheets/`、`/sheet/`、`/wiki/` 链接
