...
vi.mock("./streaming-card.js", async () => {
  ...
  return {
    ...
    FeishuStreamingSession: class {
      ...
      updateReasoning = vi.fn(async () => {});
      collapseReasoning = vi.fn(async () => {});
      close = vi.fn(async () => {
        this.active = false;
      });
...
describe("createFeishuReplyDispatcher streaming behavior", () => {
  ...
  it("does not request streamed reasoning when card streaming is disabled", () => {
  ...
  it("streams reasoning into a separate card lane and collapses it on answer start", async () => {
    resolveFeishuAccountMock.mockReturnValue({
      accountId: "main",
      appId: "app_id",
      appSecret: "app_secret",
    ...
    await options.onReplyStart?.();
    await result.replyOptions.onReasoningStream?.({ text: "先读取文件。" } as never);
    await result.replyOptions.onPartialReply?.({ text: "这是正文" } as never);
    await options.onIdle?.();

    expect(streamingInstances).toHaveLength(1);
    expect(streamingInstances[0].updateReasoning).toHaveBeenCalledWith("先读取文件。");
    expect(streamingInstances[0].collapseReasoning).toHaveBeenCalledTimes(1);
    expect(streamingInstances[0].updateAnswer).toHaveBeenCalledWith("这是正文");
  });

  it("collapses reasoning before final-only streaming close", async () => {
    resolveFeishuAccountMock.mockReturnValue({
      accountId: "main",
      appId: "app_id",
      appSecret: "app_secret",
    ...
    await options.onReplyStart?.();
    await result.replyOptions.onReasoningStream?.({ text: "先思考一下。" } as never);
    await options.deliver({ text: "最终答案" }, { kind: "final" });

    expect(streamingInstances).toHaveLength(1);
    expect(streamingInstances[0].updateReasoning).toHaveBeenCalledWith("先思考一下。");
    expect(streamingInstances[0].close).toHaveBeenCalledTimes(1);
  ...
  it("waits one microtask for late reasoning snapshots before streaming close", async () => {
    ...
    await options.onReplyStart?.();
    const lateReasoning = Promise.resolve().then(() =>
      result.replyOptions.onReasoningStream?.({ text: "补发的完整 reasoning" } as never),
    );
    await options.deliver({ text: "最终答案" }, { kind: "final" });
    await lateReasoning;

    expect(streamingInstances).toHaveLength(1);
    expect(streamingInstances[0].updateReasoning).toHaveBeenCalledWith("补发的完整 reasoning");
    expect(streamingInstances[0].close).toHaveBeenCalledTimes(1);
    expect(streamingInstances[0].close).toHaveBeenCalledWith("最终答案");
  });
