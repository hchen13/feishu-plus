...
function extractFeishuErrorInfo(err: unknown): FeishuErrorInfo | null {
  ...
  // Check HTTP response body FIRST (axios errors: actual Feishu code/msg lives in response.data)
  // This must happen before checking err.message to avoid returning the generic axios message.
  const responseData = (obj.response as { data?: unknown } | undefined)?.data;
  if (responseData) {
    const responseInfo = extractFeishuErrorInfo(responseData);
