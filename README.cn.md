...
**命令权限控制（Command Control）** 是面向企业多人部署的 slash command 分组授权系统。在实际企业场景里，不是所有人都应该能执行所有命令——`/model`、`/clear` 这类操作应该只对管理员和高权限用户开放，普通员工只需要基础命令。Command Control 允许你定义可直接映射企业组织架构的用户组，为每个组分配白名单、黑名单或全放行策略，配置保存后立即热重载生效，无需重启 gateway。
